...
q0_ref = [1,0,0,0]
w0_ref = [0,0,0]
x_ref = np.concatenate((q0_ref, w0_ref))
P_controller = 0.01
D_controller = 0.01
last_error = 0
u_array = []
...
plt.xlabel("Time (s)")
if savefig:
    plt.savefig(f"figs/quat-comp.png", dpi=400)

plt.figure()
plt.plot(t_sim, x_array[:,0]-x_filt_array[:,0], label="q0")
plt.plot(t_sim, x_array[:,1]-x_filt_array[:,1], label="q1")
plt.plot(t_sim, x_array[:,2]-x_filt_array[:,2], label="q2")
plt.plot(t_sim, x_array[:,3]-x_filt_array[:,3], label="q3")
plt.title("Quaternion component filter errors")
plt.legend()
plt.xlabel("Time (s)")
if savefig:
    plt.savefig(f"figs/quat-err.png", dpi=400)
...
plt.xlabel("Time (s)")
if savefig:
    plt.savefig(f"figs/angular-velocity.png", dpi=400)
...
plt.legend()
if savefig:
    plt.savefig(f"figs/control-history.png", dpi=400)
plt.show()